...
metadata=pd.read_csv("/export/space3/users/ismadlsh/LCG/S4/transcriptomica/Astrocite_eOH_transcriptomic_analysis/data/GSE308880_eOH_dependence_metadata_clean.csv")
...
> Estos archivos fueron descargados mediante los enlaces SFTP de GENCODE utilizando el comando `wget` [1](#section).
...
Para el proceso de descarga y control de calidad (QC) se desarrolló un *pipeline* en Nextflow ( @fig-pipeline-overview ) que permite realizar ambas tareas simultáneamente (`src/preprocces_data.nf`) [2](#section-1).

Una vez descargados los datos, el QC reveló que las secuencias presentan un *Phred score* muy alto @fig-raw-qc-heatmap , lo que indica una alta confianza en la identificación de bases y, en general, una buena distribución en las métricas relacionadas con la calidad de la secuenciación. Esto sugiere una reacción de secuenciación adecuada, observándose únicamente advertencias (*warnings*) en las secuencias 1 de cada par respecto a la calidad por posición en el *flowcell*. Dado que esta advertencia fue uniforme entre las muestras, no representa una fuente importante de sesgo.

Sin embargo, no todas las métricas fueron óptimas, por lo que se procedió a la limpieza de las secuencias mediante el software `fastp`, el cual permite corregir aquellas regiones que no superan completamente el QC. En nuestros datos fue evidente la presencia de adaptadores @fig-raw-adapter-content , particularmente aquellos enriquecidos en secuencias repetitivas como PolyG @fig-raw-gc-content y PolyA, así como un sesgo hacia bases T al inicio de las lecturas @fig-raw-base-content . Por ello se ejecutó el siguiente paso del *pipeline* de Nextflow `src/preprocces_data.nf`, especificando las opciones **--trim_front 1** y **--trim_poly_x** [3](#section-2).

Tras la limpieza de los datos se obtuvo un nuevo reporte de MultiQC con mejores estadísticas @fig-clean-qc-heatmap. Se logró corregir parcialmente el principal problema relacionado con la presencia de adaptadores, aunque persistieron algunas excepciones en posiciones intermedias, manteniendo al mismo tiempo la mayor parte de la longitud original de las secuencias. Por su parte, el porcentaje de GC permaneció prácticamente sin cambios, lo que podría sugerir contaminación en las secuencias, aunque sin afectar significativamente los pasos posteriores. De forma similar, la duplicación y la sobre-representación de secuencias no fueron corregidas, ya que el objetivo principal del estudio es identificar cambios en la expresión génica [QC results](QC_res).
...
A pesar de que `STAR` suele considerarse un claro *trade-off* entre costo computacional y rendimiento, el desempeño real para obtener los mejores resultados continúa siendo dependiente del contexto y de los datos analizados. Por ello, se evaluó el rendimiento de ambas herramientas utilizando las secuencias previamente procesadas con `fastp`. Para ello se utilizó el mismo *pipeline* empleado durante la limpieza y descarga, el cual integra ambas herramientas para su comparación [5](#section-4).

![Relación entre la proporción de lecturas alineadas y el tiempo de procesamiento por muestra.](../results/alignments/stats/aligned_vs_time.png){#fig-aligned-vs-time fig-pos="H" width="85%" fig-align="center"}
...
![Relación entre la proporción de lecturas multimapeadas y las lecturas no alineadas, evaluando la especificidad del alineamiento.](../results/alignments/stats/Multi_mapped_vs_no_aligned.png){#fig-multimapped-vs-unaligned fig-pos="H" width="85%" fig-align="center"}
...
El *pipeline* utilizado para el alineamiento [5](#section-4) implementa también la generación de matrices de conteo a partir de los alineamientos obtenidos mediante `featureCounts`, herramienta perteneciente al paquete `subread`. Para ello se empleó el archivo de anotación descargado previamente [1](#section) junto con los parámetros mínimos necesarios para su funcionamiento.
...
```{python hisat_count_mtrx}
#| echo: false
#| warning: false
#| message: false

hi_sat_count_mtrx = pd.read_csv(
    "/export/space3/users/ismadlsh/LCG/S4/transcriptomica/Astrocite_eOH_transcriptomic_analysis/results/count_matrixes/hisat_countMatrix_clean.csv"
)

render_table(
    hi_sat_count_mtrx.head(),
...
- **counts_obj**: Corresponde al mismo objeto de entrada, pero filtrado. Es importante conservar tanto la estructura como el tipo de dato original del objeto recibido.

Code [Filtrado de los conteos](#filt_func)

#### Parseo de genes diferencialmente expresados
...
Retorna: - **list**: lsita con:

- **DE_genes** `data.frame` con los resultados del DEA, incluyendo la columna **DE** con las etiquetas `"NO"`, `"UP"` y `"DOWN"`.

- **gen_res_df** `data.frame` con las estadisticas generales (numero de gens Diferencialmente expresados)

Code [Parseo de genes diferencialmente expresados](#parse_DE)
...
En particular, al trabajar con un orden de magnitud de $10^4$ genes, un umbral de FDR de $1 \times 10^{-4}$ resulta razonable, ya que implicaría esperar al rededr de un falso positvo.

##### PCA

![Análisis de componentes principales (PCA) de la matriz de conteos generada con HISAT2, mostrando la estructura global entre muestras.](../results/DEA/PCA_pre_vi_hisat2.png){#fig-pca-hisat2 fig-pos="H" width="85%" fig-align="center"}
...
![Descomposición de la varianza explicada en la matriz de conteos HISAT2 para evaluar fuentes de variación entre muestras.](../results/DEA/VarPart_pre_vi_hisat2.png){#fig-variance-partition-hisat2 fig-pos="H" width="85%" fig-align="center"}
...
![Heatmap de expresión diferencial generado con DESeq2 a partir de la matriz HISAT2, mostrando patrones de expresión entre condiciones(FDR ≤ 0.0001 y \|LFC\| ≥ 1).](../results/DEA/Heatmap_DEA_deseq2_hisat2.png){#fig-heatmap-deseq2-hisat2 fig-pos="H" width="60%" fig-align="center"}
...
![Enriquecimiento funcional en procesos biológicos. Solo se encontró significancia en los genes sobreexpresados, principalmente relacionados con mecanismos de respuesta al daño celular (FDR ≤ 0.05 y \|LFC\| ≥ 1).](../results/functional_enrich/GO_enrich_hisat2_deseq2_BP.png){#fig-go-bp fig-pos="H" width="85%" fig-align="center"}
...
![Red de interacción generada mediante string con una confianza de interacción de 150.](../results/functional_enrich/String_analyisis_DEA_genes.png){#String-graph fig-pos="H" width="85%" fig-align="center"}
...
Todo el cosigo esta disponible en https://github.com/MaxHuesca/Astrocite_eOH_transcriptomic_analysis
...
![Flujo de pipeline de nextflow desarrollada para el preprocesamiento de los datos de secuenciación desde su descarga](../results/pipeline_over_nextflow.png){#fig-pipeline-overview fig-pos="H" width="70%" fig-align="center"}
...
```{freyre_script download_ref}
mkdir -p data/reference/

#reference fasta
wget -O data/reference/GRCm39.genome.fa.gz \
 https://ftp.ebi.ac.uk/pub/databases/gencode/Gencode_mouse/release_M38/GRCm39.genome.fa.gz 

#reference annotation
wget -O data/reference/gencode.vM38.chr_patch_hapl_scaff.annotation.gtf.gz \
 https://ftp.ebi.ac.uk/pub/databases/gencode/Gencode_mouse/release_M38/gencode.vM38.chr_patch_hapl_scaff.annotation.gtf.gz 

wget -O data/reference/gencode.vM38.transcripts.fa.gz \
https://ftp.ebi.ac.uk/pub/databases/gencode/Gencode_mouse/release_M38/gencode.vM38.transcripts.fa.gz
```
...
```{freyre_script clean_in}

#path con los datos limpios mediante fastp
data_pth="/export/space3/users/ismadlsh/LCG/\
S4/transcriptomica/Astrocite_eOH_transcriptomic_analysis/data/SRR/"
...
```{freyre_script clean_srr}
nextflow run src/preprocces_data.nf \
-c src/preprocces.config \
--outdir \
/export/space3/users/ismadlsh/LCG/S4/transcriptomica/Astrocite_eOH_transcriptomic_analysis/results \
--onlyclean true \
--metadata \
/export/space3/users/ismadlsh/LCG/S4/transcriptomica/Astrocite_eOH_transcriptomic_analysis/\
data/in_clean_srr.csv \
--trimF 1 \
...
```{freyre_script index_star}

STAR --runMode genomeGenerate --runThreadN 8 \
--genomeDir indexes/star --genomeFastaFiles GRCm39.genome.fa \
--sjdbGTFfile gencode.vM38.chr_patch_hapl_scaff.annotation.gtf --sjdbOverhang 149

```

```{freyre_script index_hisat2}
...
```{freyre_script align_in}

#path con los datos limpios mediante fastp
data_pth=\
"/export/space3/users/ismadlsh/LCG/S4/transcriptomica/\
Astrocite_eOH_transcriptomic_analysis/results/clean_data/"
...
--onlyalign true \
--metadata \
/export/space3/users/ismadlsh/LCG/S4/transcriptomica/Astrocite_eOH_transcriptomic_analysis/data\
/in_align_srr.csv \
--aligner both \
--star_index \
/export/space3/users/ismadlsh/LCG/S4/transcriptomica/Astrocite_eOH_transcriptomic_analysis/data/\
reference/indexes/star \
--hisat_index \
/export/space3/users/ismadlsh/LCG/S4/transcriptomica/Astrocite_eOH_transcriptomic_analysis/data/\
reference/indexes/hisat/hisat \
--threads 8 \
--annotation_file \
/export/space3/users/ismadlsh/LCG/S4/transcriptomica/Astrocite_eOH_transcriptomic_analysis/\
data/reference/gencode.vM38.chr_patch_hapl_scaff.annotation.gtf \
-resume \
-profile sge
```
...
![Distribución del contenido de adaptadores detectado en las lecturas crudas mediante FastQC y resumido con MultiQC. La presencia de secuencias adaptadoras residuales puede afectar la calidad del alineamiento y sesgar las estimaciones de expresión.](../results/QC/fastqc_adapter_content_plot_raw_data.png){#fig-raw-adapter-content fig-pos="H" width="85%" fig-align="center"}

![Distribución del contenido por base en lecturas crudas evaluado mediante FastQC y MultiQC, útil para identificar sesgos en la composición de nucleótidos a lo largo de las posiciones de lectura.](../results/QC/fastqc_per_base_sequence_content_plot_raw_data.png){#fig-raw-base-content fig-pos="H" width="85%" fig-align="center"}

![Distribución del contenido GC en lecturas crudas, utilizada para evaluar sesgos en la composición global de nucleótidos respecto al genoma de referencia.](../results/QC/fastqc_per_sequence_gc_content_plot_raw_data.png){#fig-raw-gc-content fig-pos="H" width="85%" fig-align="center"}

![Resumen de métricas de control de calidad en lecturas crudas mediante heatmap de MultiQC, integrando múltiples indicadores de calidad por muestra.](../results/QC/fastqc-status-check-heatmap_raw_data.png){#fig-raw-qc-heatmap fig-pos="H" width="85%" fig-align="center"}

### Clean data {#QC_clean}

![Resumen de métricas de control de calidad posterior al filtrado y limpieza de lecturas, mostrando la mejora en indicadores globales de calidad tras el preprocesamiento.](../results/QC/fastqc-status-check-heatmap_clean_data.png){#fig-clean-qc-heatmap fig-pos="H" width="85%" fig-align="center"}

## DEA

### Filtrado de los conteos {#filt_func}

```{freyre_sccript filter_func}
...
### Parseo de genes diferencialmente expresados {#parse_DE}

```{freyre_script Get_DE_func}
...
```{freyre_script volcano_func}

plot_volcano <- function(res_labes,LFC = 2,FDR = 1e-10,plot_name = "Volcano plot") {
  ...
  volcano_p <- ggplot(data = res_df,aes(x = log2FoldChange,y = -log10(padj),col = DE)
  ) +
    geom_point() +labs(title = plot_name,x = "log2 Expression fold change",y = "-log10 FDR"
    ) +coord_cartesian(xlim = c(-volcano_LFC_limit, volcano_LFC_limit),ylim = c(0, volcano_FDR_limit)
    ) +
...
```{freyre_script heat_func}

plot_heatmap <- function(significant,norm_mat,num_genes = 20,plot_name = "Heatmap") {
  #mantener solo los genes diferencialmente expresados
  significant = significant[significant$DE != "NO", ]
  if (nrow(significant) == 0) {return(NULL)}
  #Ordenarlor por su p valor ajustado
  significant_order = significant[order(significant$padj),drop = FALSE]
  ...
  zscore_top <- as.matrix(t(scale(t(mat))))
  #quitar los NAs
  zscore_top <- zscore_top[complete.cases(zscore_top), ,drop = FALSE]

  # verificar que el numero de filas sea menor que 2
  n_unique <- nrow(unique(as.data.frame(zscore_top)))
  ...
  heat_p = Heatmap(
    ...
    row_labels = rownames(zscore_top),
    name = "Z score",
...
```{freyre_script object_DE}
DE_obj <- R6Class("DE_obj",public = list(
    #================================================
    # atributes of the class
    #================================================
    #external attributes
    Count_matrix = NULL,meta_data = NULL,name_DE = NULL,
    #internal generated atributes
    vsd = NULL,PCA_p = NULL,Variance_p = NULL,N_filtered_genes = NULL,
    #DEA atributes
    DEA_DESeq2 = list(res = NULL,DE_genes = NULL,
    volcano_plot = NULL,heatmap = NULL,norm_mat = NULL,gen_res = NULL
    ),
    DEA_edgeR = list(res = NULL,DE_genes = NULL,
      volcano_plot = NULL,heatmap = NULL,norm_mat = NULL,gen_res = NULL
    ),
    #atributes provided by the user
    model = NULL,contrast = NULL,LFC = NULL,FDR = NULL,
    #================================================
    # Constructor
    #================================================
    initialize = function(Count_matrix,meta_data,name_DE = NULL) {
    ...
    pre_vi = function(var_main, var_covariates = NULL) {
      ...
      dds = DESeqDataSetFromMatrix(countData = round(self$Count_matrix),
      colData = self$meta_data,
        design = self$model)
      dds = filter_by_expression(dds,group = filter_g)
      vsd = vst(dds)
      # ================================================
      # PCA
      # ================================================
      ...
      if (is.null(var_covariates) || length(var_covariates) == 0) {
        pca_plot <- ggplot(pca_data,aes(PC1,PC2,color = .data[[var_main]])
        ) +geom_point(size = 4)
      } else {
      ...
      pca_plot <- pca_plot +
        ...
        labs(title = paste("PCA", self$name_DE))
      #================================================
      # Variance analyisis
      #================================================
      ...
      form <- as.formula(paste("~",paste(intgroup, collapse = " + ")))
      varPart <- variancePartition::fitExtractVarPartModel(
        expr_matrix,form,self$meta_data)
      vp <- sortCols(varPart)
      variance_plot <- plotVarPart(vp) +
        labs(title = paste("Variance Explained", self$name_DE))
    ...
    dea_deseq2 = function() {
      ...
      #obtain the DE_genes
      res_DE_genes = get_DE_genes(res,LFC = self$LFC,FDR = self$FDR,
    ...
    dea_edgeR = function() {
      ...
      #obtain the DE_genes
      res_DE_genes = get_DE_genes(res_edgeR,LFC = self$LFC,FDR = self$FDR,
...
make_multiplot <- function(plots_list, name_plot, colN = 4) {
  ...
  plots_list <- lapply(plots_list, function(p) {
    ...
    if (inherits(p, "Heatmap") || inherits(p, "HeatmapList")) {
      return(grid::grid.grabExpr(ComplexHeatmap::draw(p))
      )
    }
    # ggplot
    ...
    stop("Tipo de plot no soportado: ",
...
### DEA principal {#DEA_prin}

```{freyre_script make_DEA}
...
contrast = makeContrasts(condition_con = treatmentDependent - treatmentNaive,
  levels = design_add
)
...
### edgeR hisat2 {#DEA_hisat2_edgeR}

```{freyre_script hisat_DEA_edgeR}
...
gene_list_gsea = gene_list_all[
  order(-gene_list_all$stat, na.last = TRUE), ,drop = FALSE]
...
gene_list_filtered <- filtered_DF[
  order(-filtered_DF$log2FoldChange, na.last = TRUE), ,drop = FALSE
]
```

### GO {#Func_en_GO}

```{freyre_script go_analyis}
...
![Enriquecimiento de términos de compartimiento celular. Fue la única categoría en la que se encontraron términos enriquecidos para los genes subregulados en el fenotipo dependiente de etanol, principalmente relacionados con procesos de división celular. Por su parte, los genes sobreregulados mostraron enriquecimientos concordantes con los observados en la ontología de procesos biológicos (FDR ≤ 0.05 y \|LFC\| ≥ 1).](../results/functional_enrich/GO_enrich_hisat2_deseq2_CC.png){#fig-go-cc fig-pos="H" width="85%" fig-align="center"}

![Enriquecimiento de términos de función molecular. Solo se encontraron términos enriquecidos en los genes sobreregulados, cuyos resultados fueron consistentes con los observados en la ontología de procesos biológicos (FDR ≤ 0.05 y \|LFC\| ≥ 1).](../results/functional_enrich/GO_enrich_hisat2_deseq2_MF.png){#fig-go-mf fig-pos="H" width="85%" fig-align="center"}

\newpage

### GSEA {#Func_en_GSEA}

```{freyre_script GSEA_analysis}
